...
class MCLSRNDCGMetric(BaseMetric, config_name="mclsr-ndcg"):
    ...
    def __call__(self, inputs, pred_prefix, labels_prefix):
        ...
        actual_k = min(self._k, predictions.shape[1])
        predictions = predictions[:, :actual_k]  # (batch_size, actual_k)
        ...
        for i in range(predictions.shape[0]):
            user_predictions = predictions[i]  # (actual_k,)
            num_user_labels = labels_lengths[i]
            user_labels = labels_flat[offset : offset + num_user_labels]
            offset += num_user_labels

            hits_mask = torch.isin(user_predictions, user_labels)  # (actual_k,)

            # Используем actual_k вместо self._k
            positions = torch.arange(2, actual_k + 2, device=predictions.device)
            weights = 1 / torch.log2(positions.float())
            dcg = (hits_mask.float() * weights).sum()

            num_ideal_hits = min(actual_k, num_user_labels)
